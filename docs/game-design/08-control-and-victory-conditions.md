...
- **Market Square:** `C3` and `D3` form the central control pair. A side controls the pair only when it has a credible infantry presence in **both** sectors. If opposing force markers contest either required sector, the pair is contested and is not controlled by either side. If neither side has an eligible presence across the pair, it is not controlled by either side.
