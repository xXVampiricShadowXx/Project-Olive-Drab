...
This is the working scenario for the first playable rules test. It is intentionally abstract and fictionalized. Its purpose is to test command, movement, information, and engagement systems, not to make a definitive claim about the real capabilities of any country.
...
For rules purposes, **unit** means a subordinate organizational element under the current commander's control. Thus, for the company commander, each echelon is a unit that can receive an order. If a subordinate commander is later placed in an echelon, that commander treats the echelon's two platoons as their units. The same rule continues downward: a platoon commander controls two sections, a section commander controls two squads, and a squad commander controls two fireteams. Terminology can vary among real militaries; this prototype chooses a fixed hierarchy so the rules do not depend on national organizational naming. This approach is consistent with the general military-organization distinction between higher formations/commands and subordinate units, while deliberately fixing the game's own structure for repeatability.
